 ...
 * @author aa
 */
public class NewClass implements EventHandler<KeyEvent> {

    private ComboBox comboBox;
    private StringBuilder sb;
    private int lastLength;

    public NewClass(ComboBox comboBox) {
        this.comboBox = comboBox;
        sb = new StringBuilder();

        this.comboBox.setEditable(true);
        this.comboBox.setOnKeyReleased(NewClass.this);
    ...
    private void selectClosestResultBasedOnTextFieldValue(boolean affect, boolean inFocus) {
        ObservableList items = NewClass.this.comboBox.getItems();
        boolean found = false;
        for (int i = 0; i < items.size(); i++) {
            if (NewClass.this.comboBox.getEditor().getText().toLowerCase().equals(items.get(i).toString().toLowerCase())) {
                try {
                    ListView lv = ((ComboBoxListViewSkin) NewClass.this.comboBox.getSkin()).getListView();
